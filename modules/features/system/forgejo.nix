...
in {
  ...
  system.activationScripts.forgejoTheme.text = lib.mkIf config.services.forgejo.enable ''
    ...
    cp ${libs.root "/assets/forgejo/logo.svg"} ${config.services.forgejo.customDir}/public/assets/img
    cp ${libs.root "/assets/forgejo/favicon.svg"} ${config.services.forgejo.customDir}/public/assets/img
  '';
}
